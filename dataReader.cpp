...
void read_config(metaData*& head, const std::string& filename) {
  ...
  head->class_count = std::max((long unsigned)(head->features_meta[head->feature_count].Max -
    head->features_meta[head->feature_count].Min) + 1,
    head->features_meta[head->feature_count].stringSet.size());
  file.close();
}
