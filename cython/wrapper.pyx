...
cdef extern from "../randomForestRegressor.hpp":
    cdef cppclass randomForestRegressor:
        randomForestRegressor() except +
        randomForestRegressor(unsigned min_samples_split, 
                             unsigned n_trees) except +
        
        void fit(float* features, float* labels) except +
        void fit(float* features, float* labels, 
                unsigned sample_count_, 
                unsigned feature_count_) except +
        
        void predict(float* features, float* labels, 
                    unsigned sample_count) except +

# Python包装类
cdef class PyRandomForestRegressor:
  cdef randomForestRegressor* thisptr  # 指向C++对象的指针
  
  def __cinit__(self, unsigned min_samples_split=5, 
                unsigned n_trees=100):
    self.thisptr = new randomForestRegressor(
  ...
  def fit(self, X, y):
    ...
    cdef cnp.ndarray[cnp.float32_t, ndim=1] y_arr = np.ascontiguousarray(np.asarray(y, dtype=np.float32), dtype=np.float32)
    
    if y_arr.shape[0] != X_arr.shape[0]:
        raise ValueError("X和y的样本数量不匹配")
    
    cdef float[:, ::1] X_view = X_arr
    cdef float[::1] y_view = y_arr
  ...
  def predict(self, X):
    ...
    cdef float[:, ::1] X_view = X_arr
    cdef cnp.ndarray[cnp.float32_t, ndim=1] y_pred = np.empty(
        X_arr.shape[0], dtype=np.float32)
    cdef float[::1] y_pred_view = y_pred
